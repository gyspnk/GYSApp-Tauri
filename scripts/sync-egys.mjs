...
async function extractContract(checkout, sourceCommit) {
  ...
  const openApiConfig = (await exists(openApiConfigPath))
    ? await readFile(openApiConfigPath, "utf8")
    : "";
  const applicationConfig = (await exists(applicationConfigPath))
    ? await readFile(applicationConfigPath, "utf8")
    : "";
  ...
  const openApiEnabledBy =
    openApiConfig.match(
      /@ConditionalOnProperty\s*\(\s*name\s*=\s*"([^"]+)"/,
    )?.[1] ?? "springdoc.api-docs.enabled";
  const openApiDocsPath =
    applicationConfig.match(/(?:^|\n)\s*path:\s*(\/[^\s#]+)/)?.[1] ??
    "/v3/api-docs";
  const openApiUiPath =
    applicationConfig.match(/swagger-ui:\s*[\s\S]*?path:\s*(\/[^\s#]+)/)?.[1] ??
    "/swagger-ui.html";
  const openApi = {
    available: Boolean(openApiConfig),
    source: openApiConfig
      ? "runtime-generated"
      : "not-present-in-upstream-checkout",
    docsPath: openApiDocsPath,
    uiPath: openApiUiPath,
    enabledBy: openApiEnabledBy,

...
- Fixed stale GitHub Pages shells by versioning the service-worker cache to
  v11 and refreshing HTML navigations from the network before falling back to
  offline cache. Synchronized Linux and Windows Kidung visual baselines so the
  latest UI changes pass the 52-flow Playwright matrix on both environments.

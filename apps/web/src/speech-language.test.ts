...
describe("automatic speech language selection", () => {
  it("detects Indonesian, English, and Chinese text without a caller hint", () => {
    ...
    expect(detect("Pada mulanya Allah menciptakan langit dan bumi.")).toBe(
      "id-ID",
    );
    expect(
      detect("In the beginning God created the heavens and the earth."),
    ).toBe("en-US");
  ...
  it("uses an explicit Bible language hint over heuristic detection", () => {
    ...
    expect(
      resolve(voices, "id-ID-GadisNeural", "God is good", "zh-CN"),
    ).toEqual({
      languageTag: "zh-CN",
  ...
  it("chooses a language-matching voice per text instead of forcing the saved voice", () => {
    ...
    const resolve = moduleUnderTest.resolveSpeechVoiceForText!;
    expect(
      resolve(voices, "id-ID-GadisNeural", "The Lord is my shepherd."),
    ).toEqual({
      languageTag: "en-US",
      voiceId: "en-US-JennyNeural",
    });
    expect(
      resolve(voices, "en-US-JennyNeural", "Tuhan adalah gembalaku."),
    ).toEqual({
      languageTag: "id-ID",
  ...
  it("falls back deterministically for short ambiguous text", () => {
    ...
    expect(moduleUnderTest.detectSpeechLanguage!("Amen", "en-US")).toBe(
      "en-US",
    );
  });
});

...
test(
  "Literature PDF rows resume directly in the internal reader",
  async ({ page }) => {
...
test(
  "Dasar Kepercayaan rows open the matching internal PDF directly and expose resume progress",
  async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem(
        "gys-faith-pdf-1",
    ...
    await expect(page.locator(".faith-pdf-stats")).toContainText(
      /Halaman 4\/10/,
    );
...
test(
  "Sauh loading state is centered in the devotion panel body",
  async ({ page }) => {
